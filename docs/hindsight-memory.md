...
retention, cancellation, failures, groups and administrator authorization without
...
Validated on 2026-09-07 with Node 24.20.0 and pnpm 10.33.0:

- `pnpm typecheck`, `pnpm test`, `pnpm build`, and `pnpm check:electron` completed.
- Main suite: 4,284 passed, 39 skipped, one TODO. Broker: eight passed.
  Electron: 146 passed. The packaged-server smoke check passed.
  Neither Hindsight test file contains skipped tests.
- An isolated browser fixture verified saving, read-only connection testing,
  submission after a direct turn, bank exclusivity, bot switching during a pending
  read, key clearing, disabling, and compact layout, with no browser errors.
- No real Hindsight installation was configured or contacted for this validation.
