...
message in OpenMausBot remains unchanged. If an expired provider session must
restart, its replacement receives the same recalled context.
...
Groups, peer delegations, bot-authored thread openings, routines, webhooks and
automatic card continuations do not use this integration. A queued turn that
combines bot-authored opening text and human input is excluded as well; later
human replies in that thread use memory normally. The existing local `MEMORY.md`, topic files and history
