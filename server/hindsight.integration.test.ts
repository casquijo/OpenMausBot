...
const TEST_CAPABILITY_KEY = "hindsight-fixture-internal-capability";
...
describe("Hindsight direct conversations (isolated HTTP integration)", () => {
  ...
  const createBot = async (name: string, instanceId = "claude") => {
    const created = await api("POST", "/api/bots", {
      name, modelSelection: { instanceId, model: "claude-fake" },
    });
  ...
  beforeAll(async () => {
    ...
    writeFileSync(join(dataDir, "config.json"), JSON.stringify({ instances: {
      claude: instance,
      second: instance,
      recovery: { ...instance, environment: { ...instance.environment, FAKE_CLAUDE_MODE: "dead-session" } },
    } }));
  ...
  it("keeps recalled memory in the rebuilt prompt when a native session can no longer resume", async () => {
    const bank = "resume-recovery";
    const bot = await createBot("Memory recovery owner", "recovery");
    const earlier = "My earlier conversation survives a missing native session";
    await send(bot.id, earlier);
    await configure(bot.id, bank);
    // A changed stable persona restarts the CLI with its existing cursor.
    // The fixture rejects --resume before init, then accepts a fresh session.
    expect((await api("PATCH", `/api/bots/${bot.id}`, { name: "Renamed recovery owner" })).status).toBe(200);
    const current = "Use my remembered preferences for this answer";
    await send(bot.id, current);
    await waitRequest(bank, "/memories");
    const seen = JSON.parse(readFileSync(dump, "utf8"));
    const prompt = JSON.stringify(seen.prompt);
    expect(seen.argv).not.toContain("--resume");
    expect(prompt).toContain("could not be resumed");
    ...
    expect(forBank(bank, "/memories/recall")[0].body.query).toBe(current);
    expect(forBank(bank, "/memories")).toHaveLength(1);
    expect(forBank(bank, "/memories")[0].body.items[0].content).toBe(`User: ${current}\n\nAssistant (Renamed recovery owner): ${REPLY}`);
  }, 30_000);
  ...
  it.each(["immediate", "capacity-queued", "mixed-capacity-queued"])(
    "does not treat a bot's own %s thread opening as a human memory exchange",
    async (mode) => {
      const bank = `self-open-${mode}`;
      const bot = await createBot("Self-opening memory owner");
      await configure(bot.id, bank);
      ...
      try {
        if (queued) {
          expect((await api("PATCH", "/api/config", { threads: { maxConcurrentPerBot: 1 } })).status).toBe(200);
          const sibling = await api("POST", `/api/bots/${bot.id}/tasks`, { title: "Human conversation holding capacity" });
          expect(sibling.status).toBe(201);
          activeThread = sibling.body.task.threadId;
          heldBanks.add(bank);
          expect((await api("POST", `/api/bots/${bot.id}/messages`, {
            threadId: activeThread, text: "The human's separate question",
          })).status).toBe(202);
          await waitRequest(bank, "/memories/recall");
        }
        const minted = await api("POST", "/api/testing/internal-capability", {
          botId: bot.id, threadId: bot.threadId, kind: "agents",
        }, { "x-openmausbot-test-capability": TEST_CAPABILITY_KEY });
        expect(minted.status).toBe(201);
        const opened = await api("POST", "/api/internal/threads", {
          title: "Independent bot work", message: "A bot-authored plan must not become a fact about the person",
        }, { authorization: `Bearer ${minted.body.token}` });
        expect(opened.status).toBe(201);
        expect(opened.body).toMatchObject({ self: true, state: queued ? "queued" : "running" });
        const threadId = opened.body.threadId;
        if (mode === "mixed-capacity-queued") {
          const followup = await api("POST", `/api/bots/${bot.id}/messages`, {
            threadId, text: "A human adds a line before the bot-authored opening drains",
          });
          expect(followup.body).toMatchObject({ queued: true, reason: "capacity" });
        }
        releaseRecall(bank);
        await expect.poll(async () => (await threadMessages(threadId)).some((message: any) =>
          message.role === "bot" && message.kind === "text" && message.text === REPLY), { timeout: 15_000 }).toBe(true);
        await waitThreadSettled(bot.id, threadId);
        if (activeThread) {
          await waitThreadSettled(bot.id, activeThread);
          await waitRequest(bank, "/memories");
        }
        const expectedHumanTurns = queued ? 1 : 0;
        expect(forBank(bank, "/memories/recall")).toHaveLength(expectedHumanTurns);
        expect(forBank(bank, "/memories")).toHaveLength(expectedHumanTurns);

        // The exclusion belongs to the bot-authored turn, not the whole
        // task: a later human message in that task still gets memory.
        const human = "Now the person asks a genuine follow-up in this thread";
        expect((await api("POST", `/api/bots/${bot.id}/messages`, { threadId, text: human })).status).toBe(202);
        await waitRequest(bank, "/memories", expectedHumanTurns + 1);
        await waitThreadSettled(bot.id, threadId);
        expect(forBank(bank, "/memories/recall").at(-1)?.body.query).toBe(human);
        expect(forBank(bank, "/memories").at(-1)?.body.items[0].content).toBe(`User: ${human}\n\nAssistant (${bot.name}): ${REPLY}`);
      } finally {
        releaseRecall(bank);
