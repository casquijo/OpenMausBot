...
function cancelHindsightTurn(botId: string, threadId?: string) {
  const turn = hindsightTurns.get(botId);
  if (!turn || (threadId !== undefined && turn.threadId !== threadId)) return;
  hindsightTurns.delete(botId);
...
function cancelDirectTurnDispatch(botId: string, expectedThreadId?: string): DirectTurnDispatchClaim | null {
  cancelHindsightTurn(botId, expectedThreadId);
  const claim = directTurnDispatchClaims.get(botId);
...
bus.subscribe((event: RuntimeEvent) => {
  ...
  switch (event.type) {
    case "turn.started": {
      const memory = bot ? hindsightTurns.get(bot.id) : undefined;
      if (bot && memory && memory.threadId === event.threadId && memory.providerInstanceId === event.providerInstanceId
        && directTurnDispatchClaims.get(bot.id)?.id === memory.claimId) {
  ...
  void (async () => {
    try {
      ...
      if (memoryConnection?.config.enabled && commsDepth === 0 && !opts?.peerAsk
        ...
        cancelHindsightTurn(bot.id);
        const controller = new AbortController();
        memoryConnection.requests.add(controller);
        const memory: HindsightTurn = {
          connection: memoryConnection, controller, threadId, claimId: dispatchClaimId,
          providerInstanceId: instanceId, userText: resolvedImages.text,
          timestamp: new Date(userMessage.at).toISOString(),
        };
        hindsightTurns.set(bot.id, memory);
        try {
          hindsightContext = await recallHindsight(memoryConnection.config, resolvedImages.text, controller.signal);
      ...
      const dispatch = await guardTurnDispatch(instance.adapter.sendTurn({
        ...
        text: hindsightContext && hindsightTurns.get(bot.id)?.claimId === dispatchClaimId
          ? `${hindsightContext}\n\n${turnText}` : turnText,
        images: turnImages,
      ...
      const memory = hindsightTurns.get(bot.id);
      ...
      if (hindsightTurns.get(bot.id)?.claimId === dispatchClaimId) cancelHindsightTurn(bot.id);
...
async function reloadProviders() {
  ...
  for (const botId of hindsightTurns.keys()) cancelHindsightTurn(botId);
